...
# Налаштування папки для зображень
...
def add_robot():
    ...
    for image in images:
        if image and image.mimetype.startswith('image/'):
            ...
            image.save(filename)
            image_paths.append(f'/static_images/{image.filename}')
        else:
...
# Обробка статичних файлів у корені (index.html, index.css, index.js)
@app.route('/')
